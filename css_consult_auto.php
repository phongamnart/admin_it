...
if (is_array($input)) {
    ...
    $result = $conDB->sqlQuery("SHOW COLUMNS FROM `tb_consult`");
    $validColumns = [];
    ...
    if (empty($masterID)) {
        echo "Skipping due to missing MasterID\n";
        exit;
    }

    $sql_check = "SELECT COUNT(*) AS count FROM `tb_consult` WHERE `MasterID` = '" . $conDB->escape($masterID) . "'";
    $checkResult = $conDB->sqlQuery($sql_check);
    $checkRow = mysqli_fetch_assoc($checkResult);

    if ($checkRow['count'] > 0) {
        $sql_update = "UPDATE `tb_consult` SET " . implode(", ", $updateFields) . " WHERE `MasterID` = '" . $conDB->escape($masterID) . "'";
        ...
        if (!empty($columns) && !empty($values)) {
            $sql_insert = "INSERT INTO `tb_consult` (" . implode(", ", $columns) . ") VALUES (" . implode(", ", $values) . ")";
